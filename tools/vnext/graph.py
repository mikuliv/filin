...
def build_interaction_graph(observations: list[dict[str, Any]], event_windows: list[list[dict[str, Any]]], detections: list[dict[str, Any]] | None = None, *, docker_evidence_refs: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for index, (observation, events) in enumerate(zip(observations, event_windows)):
        ...
        nodes[oid] = {"node_id": oid, "node_type": "observation", "entity_type": None, "canonical_ref": observation["canonical_digest"]}
        if index:
        ...
        for event in events:
            ...
            for ref in event["entity_refs"]:
                nodes.setdefault(ref["entity_id"], {"node_id": ref["entity_id"], "node_type": "entity", "entity_type": ref["entity_type"], "canonical_ref": ref["entity_id"].removeprefix("ent_")})
            ...
            if event["event_type"] == "network.flow" and source and destination:
                ...
                service = _entity_id("service", f"{destination}:{event['payload']['destination'].get('port', 0)}")
                nodes.setdefault(service, {"node_id": service, "node_type": "entity", "entity_type": "service", "canonical_ref": service.removeprefix("ent_")})
                _add_edge(edges, source, service, "ACCESSED_SERVICE", timestamp, oid)
            elif event["event_type"] == "http.request" and source:
                url = _entity_id("url", f"{event['payload'].get('host','')}{event['payload'].get('path','')}")
                nodes.setdefault(url, {"node_id": url, "node_type": "entity", "entity_type": "url", "canonical_ref": url.removeprefix("ent_")})
                ...
                nodes.setdefault(account, {"node_id": account, "node_type": "entity", "entity_type": "account", "canonical_ref": account.removeprefix("ent_")})
                if service:
                    nodes.setdefault(service, {"node_id": service, "node_type": "entity", "entity_type": "service", "canonical_ref": service.removeprefix("ent_")})
                    _add_edge(edges, account, service, "AUTHENTICATED_TO", timestamp, oid)
            elif event["event_type"] == "dns.query" and source:
                domain = _entity_id("domain", event["payload"]["question_name"])
                nodes.setdefault(domain, {"node_id": domain, "node_type": "entity", "entity_type": "domain", "canonical_ref": domain.removeprefix("ent_")})
    ...
    for detection, observation in zip(detections or [], observations):
        ...
        nodes[did] = {"node_id": did, "node_type": "detection_result", "entity_type": None, "canonical_ref": detection["canonical_digest"]}
    ...
    for item in docker_refs:
        for container_id in item.get("container_ids", []):
            entity = _entity_id("container", container_id)
            nodes.setdefault(entity, {"node_id": entity, "node_type": "entity", "entity_type": "container", "canonical_ref": canonical_digest({"container_id": container_id})})
...
def attach_reasoning_nodes(graph: dict[str, Any], correlation: dict[str, Any], incident: dict[str, Any]) -> dict[str, Any]:
    ...
    nodes[correlation["correlation_id"]] = {"node_id":correlation["correlation_id"],"node_type":"correlation_result","entity_type":None,"canonical_ref":correlation["canonical_digest"]}
    nodes[incident["candidate_id"]] = {"node_id":incident["candidate_id"],"node_type":"incident_candidate","entity_type":None,"canonical_ref":incident["canonical_digest"]}
