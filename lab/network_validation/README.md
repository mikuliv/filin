...
- `contracts.py` проверяет сценарии, события, маркер и манифест захвата;
...
- `planning.py` проверяет пары, разбиение целыми сессиями и риски косвенных признаков;
...
Сырые PCAP и журналы Zeek не редактируются. Маркер исключается только во временной копии входа модели; DNS не меняется. UID, одновременно связанный с маркером и сценарием, даёт `processing_integrity_failure`. Полное описание: [актуальная методика](../../docs/research/independent-network-validation.md).
