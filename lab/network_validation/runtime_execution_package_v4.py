...
PREDECESSOR_PACKAGE_PATH = EXECUTION / "official_execution_package_v3.json"
OFFICIAL_RUNTIME_PACKAGE_V4_PATH = EXECUTION / "official_execution_package_v4.json"
...
PACKAGE_SCHEMA = "network_validation_official_runtime_execution_package_v4"
PACKAGE_GENERATION = "superseding_operational_runtime_v4"
SUPERSEDING_REASON = "secret_acl_allowlist_enforcement"
PREDECESSOR_PACKAGE_ID = "network-validation-execution-dfbddfe6b9bce845"
PREDECESSOR_PACKAGE_DIGEST = "dfbddfe6b9bce845d0d218cd8dee29b2763eb5ee03d7ff285dcf780b0b2400cc"
...
def validate_runtime_sources_commit(sha: str) -> None:
    if not HEX40.fullmatch(sha) or _git("cat-file", "-e", f"{sha}^{{commit}}").returncode:
        raise ContractError("runtime v4 sources commit is invalid")
    if not all(_commit_contains_current_file(sha, path) for path in RUNTIME_SOURCE_PATHS):
        raise ContractError("runtime v4 sources commit does not contain current runtime sources")
...
def validate_predecessor_package() -> dict[str, Any]:
    ...
    if value.get("package_id") != PREDECESSOR_PACKAGE_ID or value.get("canonical_digest") != PREDECESSOR_PACKAGE_DIGEST:
        raise ContractError("runtime v3 predecessor identity mismatch")
    if value["canonical_digest"] != digest(_identity(value)):
        raise ContractError("runtime v3 predecessor canonical digest mismatch")
    source = value.get("runtime_sources_commit_sha", "")
    if not HEX40.fullmatch(source) or _git("cat-file", "-e", f"{source}^{{commit}}").returncode:
        raise ContractError("runtime v3 source commit is unavailable")
    historical_paths = {
        ...
        "runtime_package_builder_sha256": "lab/network_validation/runtime_execution_package.py",
    ...
    for field, relative in historical_paths.items():
        ...
        if result.returncode:
            raise ContractError(f"runtime v3 source missing: {relative}")
        ...
        if observed != value[field]:
            raise ContractError(f"runtime v3 source digest mismatch: {field}")
    return value
...
def build_preview(runtime_sources_commit_sha: str | None = None) -> dict[str, Any]:
    if runtime_sources_commit_sha is None:
        ...
        return {
            ...
            "blockers": [
                "runtime_v4_sources_not_committed",
                "official_runtime_execution_package_v4_not_created",
    ...
    return {
        ...
        "official_runtime_execution_package_valid": False,
        "candidate": value,
        "blockers": ["official_runtime_execution_package_v4_not_created", "fresh_campaign_initialization_required"],
    }
...
def validate_package(value: dict[str, Any]) -> dict[str, Any]:
    ...
    if value != expected:
        raise ContractError("runtime v4 execution package integrity mismatch")
    if value["canonical_digest"] != digest(_identity(value)):
        raise ContractError("runtime v4 execution package canonical digest mismatch")
    if value["package_id"] == value["supersedes_package_id"]:
        raise ContractError("runtime v4 execution package reused predecessor ID")
    return value


def write_official_package(path: Path, runtime_sources_commit_sha: str, created_at: str, confirmed: bool) -> dict[str, Any]:
    if not confirmed:
        raise ContractError("official runtime v4 execution package requires explicit confirmation")
    if path.exists():
        raise ContractError("official runtime v4 execution package cannot be overwritten")
...
def validate_official_package(path: Path = OFFICIAL_RUNTIME_PACKAGE_V4_PATH) -> dict[str, Any]:
