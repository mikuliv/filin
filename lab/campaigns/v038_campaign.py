"""Детерминированное трёхоконное episode-планирование кампаний v0.3.8."""
...
def load(path: Path) -> dict:
    ...
    expected = 12 if campaign["role"] == "evidence_training" else 6
    if len(campaign["runs"]) != expected or len({row["run_id"] for row in campaign["runs"]}) != expected:
        raise ValueError("Некорректное число или повтор run_id v0.3.8")
    return campaign
...
def selected_scenarios(items: list[dict], run_index: int) -> list[dict]:
    ...
    if len(selected) != 7:
        raise ValueError("Balance schedule обязан выбрать 7 benign scenarios")
    return selected
...
def build_manifest(root: Path, campaign: dict, run: dict) -> dict:
    ...
    episodes.extend((attack_scenario(root, scenario_id), False, f"{run['run_id']}:attack:{label}", label, scenario_id) for label, scenario_id in ATTACK_IDS.items())
    ...
    if len(rows) != 42:
        raise ValueError("Run v0.3.8 обязан содержать 42 окна")
    return {
