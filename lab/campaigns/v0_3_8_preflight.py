"""Preflight протокола, Docker lab и episode-планов v0.3.8."""
...
def audit_campaign(path: Path, validation: bool, candidate_freeze: Path | None) -> dict:
    ...
    for manifest in manifests:
        ...
        if len(scenarios) != 42 or len(scored) != 36 or len({row["episode_id"] for row in scored}) != 12:
    ...
    subprocess.run(["docker", "compose", "-p", "filin_v038_preflight", "-f", str(compose), "config", "--quiet"], cwd=ROOT / "lab/docker", check=True)
    ...
    return {
        ...
        "zeek_processing_configured": True, "marker_pairs_expected_per_run": 42, "episode_mapping_valid": True,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Выполнить preflight v0.3.8")
