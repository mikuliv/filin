"""Preflight протокола, Docker lab и episode-планов v0.3.10."""
...
from v0310_campaign import build_manifest, load
...
def audit_campaign(path: Path, validation: bool, candidate_freeze: Path | None) -> dict:
    ...
    for manifest in manifests:
        ...
        if len(scenarios) != 60 or len(scored) != 54 or len({row["episode_id"] for row in scored}) != 18:
    ...
    subprocess.run(["docker", "compose", "-p", "filin_v0310_preflight", "-f", str(compose), "config", "--quiet"], cwd=ROOT / "lab/docker", check=True)
    ...
    return {
        ...
        "zeek_processing_configured": True, "marker_pairs_expected_per_run": 60, "episode_mapping_valid": True,
        "warmup_isolation_valid": True, "state_reset_valid": True, "control_profile_valid": True,
        "activity_key_valid": True, "pending_reset_valid": True, "dedup_reset_valid": True,
        "strong_path_valid": True, "weak_path_valid": True, "ambiguity_path_valid": True,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Выполнить preflight v0.3.10")
