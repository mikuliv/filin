"""Последовательный resumable Docker runner кампаний v0.3.8."""
...
from v038_campaign import build_manifest
...
def run(campaign: dict, row: dict, output_root: Path) -> dict:
    ...
    volume = "filin_v038_" + run_id.lower()
    environment = {**os.environ, "FILIN_SENSOR_CAPTURE_VOLUME": volume, "COMPOSE_PROJECT_NAME": "filin_v038_lab"}
    ...
    os.environ["COMPOSE_PROJECT_NAME"] = "filin_v038_lab"
    try:
        ...
        if failed or skipped or done != 42:
            raise RuntimeError(f"Выполнено={done}/42, ошибок={failed}, пропущено={skipped}")
    finally:
    ...
    if len(captures) != 42 or any(path.stat().st_size <= 24 for path in captures):
        raise ValueError(f"Ожидалось 42 непустых PCAP, получено {len(captures)}")
    ...
    if len(full) != 42 or len(scored) != 36 or scored.episode_id.nunique() != 12:
    ...
    atomic(run_dir / "v038_run_integrity.json", integrity)
    return {**{name: "success" for name in STATUS_FIELDS}, "run_id": run_id, "warmup_rows": 6, "scored_rows": 36, "episodes": 12, "marker_pairs": 42}
...
def execute(campaign: dict, output_root: Path, resume: bool = False, strict: bool = False) -> dict:
    ...
    if lock.exists():
        raise RuntimeError("Обнаружен активный v0.3.8 runner lock")
    ...
    if strict and not all(complete(status.get(row["run_id"], {})) for row in campaign["runs"]):
        raise RuntimeError("Не все v0.3.8 runs завершены")
    return status
