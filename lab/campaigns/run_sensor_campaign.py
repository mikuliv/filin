...
import yaml
ROOT=Path(__file__).resolve().parents[2];sys.path.insert(0,str(ROOT/'lab/tools'));sys.path.insert(0,str(ROOT/'lab/campaigns'))
...
def main():
 ...
 for i,run in enumerate(c['runs']):
  ...
  rd=out/'runs'/rid;sd=rd/'sensor';sd.mkdir(parents=True,exist_ok=True);env={**os.environ};compose=str(ROOT/'lab/docker/docker-compose.lab.yml');subprocess.run(['docker','compose','-f',compose,'up','-d','sensor-capture'],env=env,check=True)
  ...
  args=Namespace(run_dir=str(rd),scenarios=str(ROOT/'lab/scenarios'),base_time=f'2026-07-13T{10+i%8:02d}:00:00Z',schedule_mode='natural',gap_seconds=15,repeat=1,mock=False,docker=True,compose_file=compose,compose_project_dir=str(ROOT/'lab/docker'),time_scale=.05,random_seed=run['random_seed'],start_services=False,rebuild_services=False,stop_services_after_run=False,max_runtime_seconds=240,window_seconds=60)
  try:result=run_pipeline(args,meta,variants,True);subprocess.run(['docker','compose','-f',compose,'stop','sensor-capture'],env=env,check=True);internal=f'captures/{rid}/attempt_001/capture.pcap';subprocess.run(['docker','run','--rm','-v','filin_sensor_capture:/captures','busybox','sh','-c',f'mkdir -p /captures/captures/{rid}/attempt_001 && cp /captures/capture.pcap /captures/{internal}'],check=True);subprocess.run([sys.executable,str(ROOT/'lab/sensor/run_zeek.py'),'--pcap',internal,'--output-dir',str(sd/'zeek'),'--storage-backend','docker_volume','--run-id',rid,'--strict'],check=True);subprocess.run([sys.executable,str(ROOT/'lab/sensor/normalize_zeek_events.py'),'--logs-dir',str(sd/'zeek'),'--output',str(sd/'zeek_events.jsonl'),'--run-id',rid],check=True);subprocess.run([sys.executable,str(ROOT/'lab/sensor/correlate_sensor_events.py'),'--manifest',str(rd/'scenario_manifest.yaml'),'--events',str(sd/'zeek_events.jsonl'),'--output',str(sd/'normalized_sensor_events.jsonl')],check=True);subprocess.run([sys.executable,str(ROOT/'ml/features/build_network_sensor_dataset.py'),'--manifest',str(rd/'scenario_manifest.yaml'),'--events',str(sd/'normalized_sensor_events.jsonl'),'--output',str(out/'datasets'/f'windows_network_sensor_v0_3_{rid}.csv')],check=True);status[rid]='success'
