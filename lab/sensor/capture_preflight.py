...
def main():
 ...
 zeek=out/'zeek'; subprocess.run([sys.executable,str(Path(__file__).resolve().with_name('run_zeek.py')),'--pcap',pcap_path,'--output-dir',str(zeek),'--storage-backend','docker_volume','--run-id','preflight'],check=True)
