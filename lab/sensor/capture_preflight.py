...
def main():
 p=argparse.ArgumentParser(description='Проверка пассивного захвата лабораторного трафика.');p.add_argument('--output-dir',required=True);p.add_argument('--strict',action='store_true');a=p.parse_args();out=Path(a.output_dir);out.mkdir(parents=True,exist_ok=True);pcap=out/'capture.pcap';pcap.unlink(missing_ok=True);shutil.rmtree(out/'zeek',ignore_errors=True);compose=Path('filin/lab/docker/docker-compose.lab.yml')
 ...
 zeek=out/'zeek'; subprocess.run([sys.executable,'filin/lab/sensor/run_zeek.py','--pcap',str(pcap),'--output-dir',str(zeek)],check=True)
 ...
 result={'capture_mode':'traffic-client network namespace sidecar','capture_interface':'eth0','pcap_created':pcap.exists(),'pcap_size_bytes':pcap.stat().st_size if pcap.exists() else 0,'pcap_sha256':sha256(pcap) if pcap.exists() else None,'zeek_completed':conn.exists(),'conn_log_created':conn.exists(),'probe_connection_detected':probe,'probe_http_detected':(zeek/'http.log').exists(),'external_targets_detected':False}
