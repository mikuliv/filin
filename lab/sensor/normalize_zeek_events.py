...
def main():
 ...
 for path in Path(a.logs_dir).glob('*.log'):
  for ordinal, raw in enumerate(parse_zeek_log(path), start=1):events.append({'event_id':f"{a.run_id}:{path.name}:{ordinal}:{raw.get('uid','')}",'sensor_event_id':raw.get('uid'),'run_id':a.run_id,'event_source':'zeek_sensor','observation_source':'network_sensor','sensor_type':'zeek','sensor_log_type':path.stem,'zeek_uid':raw.get('uid'),'timestamp':raw.get('ts'),'execution_mode':'docker','synthetic':False,'raw':raw})
