...
class SensorArtifactStorage:
    ...
    output_volume_name:str='filin_sensor_zeek'
    ...
    def run_zeek(self,pcap_path,run_id,attempt_id='attempt_001'):
        output_volume=self.output_volume_name; command=['docker','run','--rm','-v',f'{self.volume_name}:/captures:ro','-v',f'{output_volume}:/zeek-output','zeek/zeek:7.0.5','sh','-c',f'mkdir -p /zeek-output/{run_id}/{attempt_id} && cd /zeek-output/{run_id}/{attempt_id} && zeek -C -r /captures/{pcap_path} LogAscii::use_json=T']
