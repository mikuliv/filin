...
Shadow-readiness audit пройден. Schema, privacy, reconciliation, replay, idempotency, queue, spool, crash, faults, fail-safe и resource gates положительны. Следующий этап — локальный v0.3.15; фактический shadow mode и backend integration запрещены.

## v0.3.13

Prospective blind environmental holdout завершён положительно. Кампания 10/10, capture lock 760/760, immutable prediction 700/700, causal invariance 7/7, regression bundle complete и validated. Следующий этап — v0.3.14 shadow-readiness; shadow mode и backend integration запрещены.
...
Local controlled passive shadow trial завершён и прошёл policy. Зафиксированы 10/10 sessions, 1520/1520 captures, 1440/1440 unique predictions и полная sink reconciliation. Candidate готов только к v0.3.16 staging connector readiness; shadow/backend/production остаются запрещены.

# Технический аудит v0.3.12.1

Frozen rates 22/30 и 44/60 подтверждены, но их latency positions возникли из некаузального порядка массива immutable prediction. После сортировки по frozen `causal_order` third-window alerts отсутствуют; state-machine и activity key согласованы. Статус v0.3.12 остаётся отрицательным.
