...
Объявления извлекаются из всех tracked `requirements*.txt`; локально установленные distributions и их транзитивное замыкание — через `importlib.metadata`. License-Expression, classifiers и license files рассматриваются как подтверждающие материалы; нормализация в SPDX фиксируется отдельно. Сеть и установка пакетов не используются.
