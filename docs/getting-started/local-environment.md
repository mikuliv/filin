...
Обычные тесты и консоль могут создавать файлы в `runtime/`; это изменяемый локальный слой, не подтверждающие материалы. Научный сетевой запуск имеет отдельные каталоги результатов и секретов и не должен направляться в отслеживаемую область репозитория.
...
Команда `python -m lab.network_validation.cli --help` безопасна после установки зависимостей. Команды чтения контрактов и пробной проверки перечислены в [справочнике команд](../reference/command-reference.md). Команды, запускающие единицу Phase 1, создание PCAP, сопоставления, меток, прогнозов или метрик, намеренно не приводятся как инструкция установки.
