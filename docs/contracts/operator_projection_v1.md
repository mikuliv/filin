...
Контракт определяет минимальную санитарную только для чтения проекцию durable receiver event для локальной репетиции v0.3.17. Авторитетная Файлы JSON Schema находится в `rehearsal/contracts/operator_projection_v1.schema.json`.
...
Запрещены raw IP/MAC/hostname/user/email, credentials, payload, признак vector, label, scenario/class metadata, private keys, absolute paths, traces и environment values. Проекция не содержит attack intent, MITRE mapping, свободный модельный текст, рекомендации блокировки или элементы управления действием.
