...
| `validate-freeze-candidate` | Проверка кандидата на фиксацию; `--campaign` | Только чтение |
...
| `validate-official-freeze` | Проверка официальной фиксации; `--campaign`, `--acceptance-criteria`, `--image-lock`, `--freeze` | Только чтение |
| `audit-execution-readiness` | Аудит готовности выполнения; те же четыре аргумента, что у официальной фиксации | Только чтение |
...
| `validate-official-superseding-freeze` | Проверка заменяющей фиксации; `--freeze` | Только чтение |
...
Эти команды относятся к операционному контуру. Предпросмотр обычно читает входы и формирует результат для проверки, а `materialize` и `create-official-*` могут записать операционные или официальные артефакты. Они не должны считаться научным стартом, но требуют проверки пути вывода и исходных коммитов, а также разрешения владельца.

| Команда | Назначение и аргументы | Побочный эффект |
|---|---|---|
| `build-freeze-preview` | Предпросмотр фиксации; `--campaign`, `--acceptance-criteria`, `--image-lock` | Только чтение; результат выводится в консоль |
| `create-official-freeze` | Создание официальной фиксации; те же пути плюс `--output`, подтверждение `--confirm-official-freeze` | Запись официального артефакта |
...
| `create-official-superseding-freeze` | Создание заменяющей фиксации; `--output`, `--confirm-official-freeze` | Запись официальной фиксации |
...
Консоль слушает только локальный узел и записывает изменяемый слой в `runtime/lab_console/`. Проверки цикла:
...
Успешная команда подтверждает только свой контракт. Она не означает внешнюю валидность, истинность гипотезы, прохождение научной проверки, готовность к промышленной эксплуатации, право регистрации кандидата или разрешение автоматического ответа. Фактические числа регрессии фиксируются в отчёте конкретного окружения; исходное состояние описано в [тестировании](../getting-started/testing.md).
