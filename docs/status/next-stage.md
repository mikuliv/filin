...
## Лабораторная линия: v0.4.7

После завершения `v0.4.5`, затем `v0.4.6` и ручного решения `admitted_to_separate_validation` разрешена только подготовка отдельного frozen протокола проверки предложения. Само испытание, регистрация, продвижение, активация или замена кандидата не разрешены. Лабораторная линия не подменяет основную линию `v0.3.19`.
