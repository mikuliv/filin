...
| Лабораторная `v0.4.x` | `v0.4.7` | Role-separated blind-процедура пройдена; proposal получил `failed_validation` | Независимая человеческая проверка; `v0.4.8` запрещён |
...
- proposal-контур v0.4.6 с frozen split/recipe, двумя обучениями, screening, admission gate и ручным решением без регистрации кандидата.
- blind-контур v0.4.7 с закрытой разметкой, двумя prediction commitments, детерминированной оценкой и честным решением `failed_validation` без регистрации или продвижения модели.
...
Собственный программный код распространяется по MPL-2.0, собственная документация — по CC-BY-4.0. Точный scope, исключения для моделей и данных, third-party notices и профили распространения описаны в [центре лицензирования](docs/licensing/index.md). Название и графические обозначения не предоставляются по лицензии программного кода.

Официальные тексты лицензий и DCO включены как неизменённые upstream standard texts и не являются произведениями автора проекта. Техническая готовность к распространению подтверждена только для `source-core` и `laboratory-source`; offline-контейнерный bundle не одобрен, модели и datasets требуют отдельной лицензии.

## Лабораторные запуски v0.4.5

Локальная консоль ведёт каталог воспроизводимых запусков и безопасно сравнивает их результаты. План, вход, кандидат, окружение и зависимости фиксируются до выполнения; прерванный запуск восстанавливается только явным действием оператора. Сравнение сначала проверяет сопоставимость и не выбирает победителя, не изменяет текущего кандидата и не разрешает production. Руководства: [повтор лабораторного запуска](docs/getting-started/running-laboratory-replays.md) и [сравнение запусков](docs/getting-started/comparing-laboratory-runs.md).

## Предложения лабораторных кандидатов v0.4.6

Консоль готовит proposal только из allowlist synthetic data и frozen recipe, проверяет независимое повторное обучение, выполняет internal screening и сохраняемое ручное review. Proposal не получает `candidate_id`, не входит в registry и не может быть автоматически активировано или продвинуто. Руководства: [подготовка proposal](docs/getting-started/preparing-candidate-proposals.md) и [ручное рассмотрение](docs/getting-started/reviewing-candidate-proposals.md).
