...
- воспроизводимое применение зафиксированной модели и принятие решений на уровне эпизода в проверенной лабораторной области;
...
Команда проверяет зафиксированный комплект v0.4.4 и не запускает новый исследовательский цикл.
...
|---|---|---|
| `collectors/` | сбор событий, реестр кандидатов и пассивная среда выполнения | текущий |
| `ml/` | признаки, кандидат, протоколы, кампании и зафиксированные отчёты | текущие и исторические подтверждающие материалы |
| `staging/` | изолированный эталонный приёмник событий | текущая лабораторная среда выполнения |
...
| `external_review/` | исполняемые контракты внешней процедуры | зафиксированный, ограниченный |
...
| `runtime/` | локальные изменяемые результаты запусков | по умолчанию не являются подтверждающими материалами |
...
[модель безопасности](docs/safety-model.md) и правила конкретного зафиксированного пакета.
Документация не является юридическим заключением или разрешением на испытание.

## Лицензирование

Собственный программный код распространяется по MPL-2.0, собственная документация — по CC-BY-4.0. Точная область применимости, исключения для моделей и данных, уведомления о сторонних компонентах и профили распространения описаны в [центре лицензирования](docs/licensing/index.md). Название и графические обозначения не предоставляются по лицензии программного кода.

Официальные тексты лицензий и DCO включены как неизменённые стандартные тексты upstream и не являются произведениями автора проекта. Техническая готовность к распространению подтверждена только для `source-core` и `laboratory-source`; автономный контейнерный комплект не одобрен, модели и наборы данных требуют отдельной лицензии.
...
Консоль готовит предложение кандидата только из перечня разрешённых значений синтетических данных и зафиксированного рецепта, проверяет независимое повторное обучение, выполняет предварительную внутреннюю проверку и сохраняемое ручное рассмотрение. Предложение кандидата не получает `candidate_id`, не входит в реестр и не может быть автоматически активировано или продвинуто. Руководства: [подготовка предложения кандидата](docs/getting-started/preparing-candidate-proposals.md) и [ручное рассмотрение](docs/getting-started/reviewing-candidate-proposals.md).
