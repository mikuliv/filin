...
def main():
 p=argparse.ArgumentParser(description='Внешняя оценка frozen Zeek baseline.');p.add_argument('--output-dir',required=True);a=p.parse_args();out=Path(a.output_dir);out.mkdir(parents=True,exist_ok=True);root=Path('lab/output/datasets');train=pd.concat([pd.read_csv(x) for x in root.glob('windows_network_sensor_v0_3_run_v030_zeek_train_*.csv')]);robust=pd.concat([pd.read_csv(x) for x in root.glob('windows_network_sensor_v0_3_run_v032_*.csv')]);excluded={'run_id','label','label_type','scenario_id','execution_id','scenario_execution_key','feature_profile','observation_source','sensor_type','execution_mode','synthetic'};features=[x for x in train if x not in excluded and pd.api.types.is_numeric_dtype(train[x])];model=Pipeline([('imputer',SimpleImputer(strategy='median')),('scale',StandardScaler()),('model',LogisticRegression(max_iter=1000,class_weight='balanced',random_state=42))]);model.fit(train[features],train.label);per={}
