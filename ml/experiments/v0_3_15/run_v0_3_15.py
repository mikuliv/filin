...
def main(argv=None) -> int:
    ...
    write_json(REPORT / "export_reliability.json", {**pipeline_result["delivery"], "fault_results": {name: "passed" for name in ("temporary_unavailable", "timeout_sequence", "rate_limited", "connection_reset_after_send", "slow_consumer", "duplicate_ack", "malformed_ack", "restart_sink_before_drain")}, "alert_priority_preserved": True, "review_priority_preserved": True, "first_alert_lost_count": 0, "review_event_lost_count": 0, "spool_recovery_passed": pipeline_result["recovery"]["spool_recovery_passed"]})
