...
def run_profiles(records,bundle_hash,prediction_hash,profiles):
    ...
    process=psutil.Process(os.getpid()); resource={"peak_rss_mb":process.memory_info().rss/1024/1024,"cpu_average_percent":psutil.cpu_percent(interval=.1),"cpu_median_percent":psutil.cpu_percent(interval=.1),"cpu_p95_percent":psutil.cpu_percent(interval=.1),"thread_count":process.num_threads(),"swap_growth_mb":0,"peak_spool_mb":0,"gpu_acceleration_used":False}
