...
def main() -> None:
 ...
 for run in a.runs:
  for profile in a.profiles:
   short='core' if profile=='client_core_v0_2' else 'extended';csv=Path(f'lab/output/datasets/windows_client_{short}_v0_2_{run}.csv');row={'run':run,'profile':profile,'validator_status':'failed','availability_audit_status':'success','errors':[]}
   try:
