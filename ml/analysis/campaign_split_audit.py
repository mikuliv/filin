...
def main():
 p=argparse.ArgumentParser(description='Аудит разделения train/test кампании.');p.add_argument('--output-root',default='lab/output');p.add_argument('--json-report');a=p.parse_args();r=audit_campaign_split(Path(a.output_root));write_result(Path(a.json_report) if a.json_report else None,r);print(json.dumps(r,ensure_ascii=False));raise SystemExit(0 if r['train_test_split_valid'] else 1)
