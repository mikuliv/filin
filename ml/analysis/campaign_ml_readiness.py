...
def main():
 p=argparse.ArgumentParser(description='Проверка готовности независимой кампании к ML.');p.add_argument('--output-root',default='lab/output');p.add_argument('--json-report');a=p.parse_args();r=evaluate_readiness(Path(a.output_root));write_result(Path(a.json_report) if a.json_report else None,r);print(json.dumps(r,ensure_ascii=False));raise SystemExit(0 if r['ready_for_ml'] else 1)
