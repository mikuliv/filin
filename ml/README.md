...
## Филин v0.2: Docker-наблюдения

Windows и flow датасеты могут строиться по Docker-прогонам. Поля `execution_mode`, `synthetic` и `observation_source` сохраняются для анализа происхождения, но validators исключают их из model features. Docker client observations не заменяют Zeek/Suricata flow.

## Филин v0.2.1 — Docker-to-Docker evaluation

Сравнения `Docker -> Docker` проверяют переносимость между разными прогонами одного стенда. Сравнение `mock -> Docker` показывает смену источника данных. Высокое accuracy при низком macro F1 не означает устойчивого распознавания всех классов.
# Подготовка данных v0.2.3

Готовность к ML определяется числом независимых executions, а не числом связанных окон.

В v0.2.4 обязательный DummyClassifier служит reference, а основные метрики выбора — macro F1 и attack recall.
