...
class TestResearchState(unittest.TestCase):
    def test_authoritative_state_is_safe_and_current(self):
        ...
        self.assertEqual(state["latest_completed_stage"], "v0.3.14")
        self.assertEqual(state["latest_completed_result"], "shadow_readiness_passive_integration_audit_passed")
        self.assertEqual(state["next_allowed_stage"]["kind"], "controlled_passive_shadow_trial")
