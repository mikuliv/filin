...
class Delivery:
    def __init__(self, journal: ConnectorJournal, endpoint: str, instance_id: str) -> None:
        ...
        self.queue: queue.Queue[list[dict]] = queue.Queue(maxsize=3)
        self.context = client_context(os.environ["DELIVERY_TLS_CERT"], os.environ["DELIVERY_TLS_KEY"], os.environ["DELIVERY_TLS_CA"])
        self.previous: str | None = None
        self.sequence = 0
        self.lock = threading.Lock()
        self.metrics = {"worker_count": 2, "batch_request_count": 0, "max_batch_size": 0, "retry_count": 0}
        for index in range(2):
            threading.Thread(target=self._worker, args=(index,), daemon=True, name=f"sender-{index}").start()
        pending = self.journal.pending(50)
        if pending:
            self.queue.put(pending)

    def submit(self, events: list[dict]) -> None:
        for start in range(0, len(events), 50):
            self.queue.put(events[start:start + 50], timeout=2)

    def _batch(self, events: list[dict], worker: int, attempt: int) -> dict:
        with self.lock:
            self.sequence += 1
            token = digest([self.instance_id, self.sequence, [event["event_id"] for event in events]])
            batch = {"batch_contract_version": "staging_event_batch_v1", "batch_id": "bat_" + token, "attempt_id": f"att_{token[:48]}_{attempt}", "connector_instance_id": self.instance_id, "candidate_registry_commitment_sha256": REGISTRY_COMMITMENT, "event_contract_version": "shadow_event_v2", "events": events, "event_count": len(events), "previous_batch_commitment_sha256": self.previous}
    ...
    def _worker(self, worker: int) -> None:
        while True:
            ...
            if not delivered:
                try: self.queue.put(events, timeout=1)
...
def main() -> int:
    ...
    instance = os.environ.get("CONNECTOR_INSTANCE_ID", "connector_v0316_local")
    journal = ConnectorJournal(args.db)
    delivery = Delivery(journal, args.receiver, instance)
    ...
    def ingress(value):
        ...
        accepted = [event for event in request["events"] if event["event_id"] in set(commit["accepted"])]
        if accepted:
            delivery.submit(accepted)
        ack = {"ack_contract_version": "connector_ingress_ack_v1", "request_id": request["request_id"], "connector_instance_id": instance, "connector_journal_commit_id": commit["commit_id"], "connector_journal_commit_sha256": commit["commit_sha256"], "accepted_event_ids": commit["accepted"], "duplicate_event_ids": commit["duplicates"], "rejected_events": [], "durable": True, "error_code": None}
    ...
    serve(args.host, args.port, context, {"/staging-connector/v1/events": ingress}, lambda: args.db.exists() and journal.counts()["pending"] < 3200)
    return 0
