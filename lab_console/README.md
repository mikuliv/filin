...
## Новая слепая проверка

Раздел `/ui/v0473-validations` содержит 37 русскоязычных представлений завершённой проверки v0.4.7.3. Локальный API использует строгий перечень разрешённых переходов, сохраняет состояние и аудит, но не предоставляет регистрацию, активацию, продвижение, изменение порога, переобучение, загрузку или произвольную команду. Автономная проверка: `python -m tools.lab_console.verify_v0473`.

## Новое корректирующее предложение

Раздел `/ui/corrective-proposal` предоставляет 19 представлений v0.4.7.2 только для чтения: происхождение, новые данные, непересечение, разбиение, рецепт, обучение, восстановление, воспроизводимость, артефакт, предложение, внутреннюю проверку, исправление прежних ошибок, сравнения, критерии, ручное рассмотрение, решение, ограничения и экспорт. Маршруты регистрации, активации и продвижения отсутствуют.

## Разбор отрицательного результата

Раздел `/ui/failure-analysis` предоставляет 19 сохраняемых представлений v0.4.7.1: критерии, различия, классы, сессии, ошибки, уверенность, признаки, причины, действия, запреты, автономность и готовность. Раздел доступен только для чтения и не содержит регистрации, активации, продвижения или повторного применения модели к набору v0.4.7.
